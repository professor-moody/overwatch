...
- **[83 MCP tools](tools/index.md)** — State management, recovery and config reconciliation, action logging, graph queries, BloodHound/AzureHound ingestion, 75 output parsers, recoverable interactive-session descriptors, credential playbooks, and pentest report generation. (The live count + set come from `get_system_prompt`.)
