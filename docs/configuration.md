...
| `OVERWATCH_STATE_FILE` | derived from engagement ID | Raw/setup-time durable state selection; normal managed lifecycle uses `state_file_path` from the profile |
| `OVERWATCH_RUNTIME_PROFILE` | `./.overwatch-runtime/profile.json` | Explicit profile path for lifecycle-backed stdio/client wiring |
| `OVERWATCH_SKILLS` | `./skills` | Path to skill library directory |
| `OVERWATCH_DASHBOARD_HOST` | `127.0.0.1` | Dashboard bind address; `0.0.0.0` is not a client-reachable browser hostname |
| `OVERWATCH_DASHBOARD_PORT` | `8384` | Port for live dashboard (set to `0` to disable) |
| `OVERWATCH_DASHBOARD_TOKEN` | none on loopback; generated by setup for non-loopback | Optional stopped-setup credential override; setup stores the selected value in a private token file |
...
| `OVERWATCH_MCP_TOKEN` | generated by daemon setup | Optional stopped-setup override for the `/mcp` Bearer authority; setup converges the token file and client entry |
| `OVERWATCH_ENGAGEMENT_ACTIVE` | unset | Claude-client hook gate (`1`/`true`/`yes`/`on`); does not select server config or state |
