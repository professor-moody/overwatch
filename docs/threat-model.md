...
- **Mid-write state corruption.** State persistence does atomic write-rename + per-mutation write-ahead log (`src/services/mutation-journal.ts`) for engagements with a nonce. Recovery on load: snapshot + journal-replay. Crash mid-write leaves a partial line that the reader stops at without poisoning subsequent replays.
- **Activity log tampering.** Hash chain (`src/services/activity-chain.ts`) is **default-on for new engagements** with signed checkpoints every 500 events / 30 minutes. `verify_activity_chain` walks the chain; `verifyCheckpoints` resumes from the latest checkpoint instead of replaying genesis.
...
- **Checkpoint signing key management.** `engagement_signing_key_id` slot exists on the schema but key generation, rotation, and verification are not implemented. *Roadmap: Ed25519 signing flow.*
