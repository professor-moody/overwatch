...
- a **"Needs you" strip** that surfaces what's waiting on the operator — pending **approvals** (inline Approve / Deny+reason), agent **questions** (inline Answer), and **stuck agents** (heartbeating but making no progress for a while — View agent → to steer or stop) — and hides itself when nothing needs attention. Identical questions from multiple agents **cluster** into one card: answer once and it fans out to every asking agent. *Blocked* (waiting on an approval/answer) is already represented by that approval/question item; *stuck* is the distinct "alive but idle" case the watchdog's heartbeat-TTL can't catch;
