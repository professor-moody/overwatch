...
|--------|------|------|-----------|-------------------------|
| 0.3.0 release | 2026-08-18 | Efficacy/correctness | Shipped the efficacy & evidence program — claim maturity and lifecycle, the scorecard model, canonical edge types, and evidence-provenance work — as a same-format release that removes no compatibility surface and exercises the first `0.2.0 → 0.3.0` N-1 rollback gate. | Reports state what an engagement has actually proven, and operators can upgrade and roll back across the release without a state migration. |
