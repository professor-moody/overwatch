...
export interface ReportOptions {
  ...
  /** The engagement scorecard, rendered as a first-class "Evidence Integrity" section in
   * the document body (not appended after the footer). Computed once by the assembler and
   * shared with the JSON/HTML/PDF outputs. */
  engagement_scorecard?: EngagementScorecard;
}
...
export function renderFullReportFromModel(
  ...
  if (options.engagement_scorecard && scorecardHasContent(options.engagement_scorecard)) {
    lines.push(`${include_narrative && narrative.length > 0 ? '8' : '7'}. [Evidence Integrity](#evidence-integrity)`);
  }
  lines.push('');
  ...
  // === Evidence Integrity (engagement scorecard) ===
  // A first-class section in the document body — the same shared row model the JSON and
  // HTML/PDF outputs render, never a post-footer append.
  if (options.engagement_scorecard && scorecardHasContent(options.engagement_scorecard)) {
    lines.push('## Evidence Integrity');
    lines.push('');
    lines.push(SCORECARD_INTRO);
    lines.push('');
    lines.push('| Signal | Value |');
    lines.push('|--------|-------|');
    for (const row of scorecardRows(options.engagement_scorecard)) {
