  ...
  it('keeps playbook coverage aligned across Markdown, HTML, JSON, and client-safe output', () => {
    ...
    const json = JSON.parse(assembleReport(qa.engine, qa.skills, { format: 'json', profile: 'operator' }).content) as {
      ...
      engagement_scorecard: {
        verification: { total: number; verified_share: number };
        findings: { total: number; proof_ready: number };
        inventory: { total: number; observed: number };
        attack_paths: { total: number; validated: number };
        refutation: { tested: number; coverage: number };
    ...
    // ...as a section in the document BODY, not appended after the footer.
    expect(markdown.indexOf('## Evidence Integrity')).toBeLessThan(markdown.indexOf('*Generated by Overwatch'));
    // ...and now in HTML/PDF too (PDF renders from this HTML), split into its dimensions.
    expect(html).toContain('id="evidence-integrity"');
    expect(html).toContain('Attack path validated');
    ...
    // v2 split dimensions ride the JSON report.
    expect(json.engagement_scorecard.inventory.total).toBeGreaterThan(0);
    expect(json.engagement_scorecard.attack_paths.total).toBeGreaterThan(0);
    expect(typeof json.engagement_scorecard.refutation.coverage).toBe('number');
    expect(typeof json.engagement_scorecard.unsupported_critical_claims).toBe('number');
