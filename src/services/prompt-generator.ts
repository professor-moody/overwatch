...
1. **ORIENT (always your first action)** — \`get_state()\` is the very first call of every session (and every resume after compaction), **before any \`run_bash\`, \`run_tool\`, or dispatch**. Don't open by poking the target or the shell — orient from the graph first. It is the whole briefing — scope, discoveries, access, objectives, and the scored frontier; reason from it, never from memory of a prior turn. (Overwatch tools load on demand; if one you need isn't present, find it with \`ToolSearch\` before assuming it's gone.)
...
function leanTacticsSection(agent?: AgentTask): string {
  ...
  if (agent?.archetype && CREDENTIAL_ARCHETYPES.has(agent.archetype)) {
    lines.push('- For captured cloud/SaaS credentials, prefer the durable playbook tools (`expand_aws_credential`, `expand_github_credential`, `expand_entra_credential`, `expand_oidc_capture`, `exchange_refresh_token`) over re-deriving the recon chain by hand. Expansion creates or resumes a matching run; claim exactly one ready step with `start_playbook_step` (or `retry_playbook_step`) and preserve its run/step/attempt linkage through the returned `runner` or direct `tool`. Never execute a blocked or null-command descriptor; resolve `env_from_credential` to the selected credential value in `run_bash.env`, and preserve every parser field.');
  }
  return lines.join('\n');
}
...
10. **Dispatch sub-agents** for parallel work using Overwatch's \`dispatch_agents()\` (or \`register_agent()\` for one-off). **Prefer Overwatch dispatch over the host runtime's built-in subagent/Task tool** — only Overwatch-registered agents carry a frontier_item_id, lease, and dashboard surface. A subagent spawned via the host runtime that calls \`run_bash\` directly will auto-register on first tool call (so it appears in the AgentsPanel), but it lacks skill/scope/lease metadata and will look like an anonymous worker.
   - **\`credential_test\` frontier items for TOKEN credentials are automatically executed** by the scripted runner when the dashboard is running — a token credential with a \`cred_value\` is validated via curl (\`validate_token_credential\`) through the approval gate without intervention; you do NOT need to dispatch agents or manually call \`validate_token_credential\` for these, they resolve on their own (\`get_state()\` shows results). **But a NON-token web credential_test** (e.g. a plaintext username/password against an \`http\`/\`https\` login form) is NOT auto-run — the scripted runner only auto-validates *token* credentials, so a password/hash pairing is skipped and left on the frontier. Test those yourself (or via a \`web_tester\` / \`credential_operator\` agent) with **\`test_webapp_credential\`**, passing a \`success\` criterion (a post-login redirect, a body marker, or a strict status) — otherwise the pairing sits unresolved on the frontier.
...
function generateKeyPrinciplesSection(config: EngagementConfig): string {
  const lines = [
    ...
    '- **Dashboard (four operator workflows):** **Operate** is the home for the prioritized Needs you queue, active and historical work, canonical Frontier order, campaigns, the natural-language composer, steering, and the guided **Start Work** launcher. **Investigate** keeps topology as the primary lens alongside Assets, Identity, Credentials, and Paths. **Review** groups findings by canonical proof readiness, keeps proof and producing-run context together, and owns report generation/archive. **Manage** contains engagement lifecycle, scope/objectives, settings/OPSEC, recovery, and diagnostics. Activity, Sessions, and Runs are available from the shared bottom drawer without leaving the current workspace. The persistent shell shows engagement identity, synchronization, recovery, and OPSEC posture. Existing panel URLs redirect into equivalent workspace state for the documented compatibility window.',
    '',
    '### Scope guardrails',
    '',
...
For captured cloud / SaaS credentials, prefer the **playbook tools** over re-deriving the canonical recon chain by hand. Expansion creates or resumes a durable matching run; \`new_run: true\` explicitly starts another. Claim exactly one ready step with \`start_playbook_step\` (or \`retry_playbook_step\`), honor its returned \`runner\` (\`run_bash\` or \`run_tool\`) or direct \`tool\`, and preserve the returned run/step/attempt linkage through execution and parsing. Never execute a descriptor that is blocked, \`ready: false\`, or has a null command. Re-expand after a dependency lands to materialize newly bound steps. Resolve every \`env_from_credential\` mapping to the selected credential's actual value in \`run_bash.env\` (never the credential ID), and pass \`parse_with\`, \`parser_context\`, and \`parse_stream\` through unchanged.
