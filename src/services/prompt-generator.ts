...
function generateKeyPrinciplesSection(config: EngagementConfig): string {
  const lines = [
    ...
    '- **Dashboard (console-first IA):** the **Console** is the home — the operator dispatches, steers, talks to, and approves agents there, leaving only to investigate. Nav is grouped **Console** (Console · Frontier · Approvals · Campaigns) · **Investigate** (Graph · Findings · Attack Paths · Evidence · Identity · Credentials · Activity · Overview) · **Manage** (Sessions · Engagements · Settings · Smoke). In the Console: a pinned command bar, a Fleet roster (select an agent to focus it), a "Needs you" strip for inline approve/deny + agent questions, a **Deploy** button (type an IP/CIDR/domain → it is scoped + a recommended **agent type** is dispatched at it in one step, or pick the type — recon_scanner/web_tester/credential_operator/post_exploit/cve_researcher/pathfinder/report_scribe; node IDs dispatch against existing nodes), an **Add Targets** button (paste IPs/CIDRs/domains → preview → add to scope), and the live activity stream. **Approvals** is the deep triage view (same approve/deny). **Credentials** shows captured tokens/keys with status, reachability, and reveal/copy for `cred_value`. **Findings** shows classified severity groups + report archive.',
    '',
    '### Scope guardrails',
    '',
