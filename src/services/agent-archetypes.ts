...
// Credential expansion creates durable runs. Lifecycle tools reserve one step
// at a time; the emitted runner or direct-tool descriptor still passes through
// the normal execution surface and carries the run/step/attempt linkage.
const CRED = [
...
const ARCHETYPES: Record<AgentArchetypeId, AgentArchetype> = {
  ...
  planner: {
    id: 'planner', label: 'Planner (legacy role)', role: 'planner', scopeStrategy: 'scope-wide', defaultSkill: 'operator-planner',
...
const MISSIONS: Record<AgentArchetypeId, string> = {
  ...
  credential_operator:
    `YOUR ROLE IS CREDENTIAL OPERATIONS. Validate, spray, and expand credentials and tokens, and map the access they unlock. Match the tool to the target: IdP / cloud SSO tokens → validate_token_credential + the expand_* tools; a credential against a web app (http/https) → test_webapp_credential (with an explicit success criterion). Expansion creates or resumes a durable matching playbook run. Claim exactly one ready step with start_playbook_step (or retry_playbook_step), then honor its runner (run_bash or run_tool) or direct tool and preserve the returned playbook run/step/attempt linkage. Never execute a blocked, ready:false, or null-command descriptor. For run_bash, resolve env_from_credential to the selected credential's actual value in run_bash.env — never the node id — and preserve parser_context/parse_with/parse_stream. Ingest prerequisite output and re-expand to materialize newly bound steps. Record findings with report_finding. You have NO interactive sessions — credential lifecycle only. Done when each credential's validity and the access it unlocks is recorded as findings/edges (or the credential is marked invalid).`,
  post_exploit:
  ...
  cloud_cartographer:
    `YOUR ROLE IS CLOUD + IDENTITY CARTOGRAPHY. Expand captured cloud credentials/tokens into durable playbook runs (AWS/Entra/GitHub/OIDC). Claim and execute exactly one ready step at a time, preserving its run/step/attempt linkage through validate_action and the returned runner (run_bash or run_tool) or direct tool. Never execute a blocked, ready:false, or null-command descriptor. Resolve env_from_credential to actual credential material in run_bash.env, preserve parser_context/parse_with/parse_stream, ingest prerequisite output, and re-expand to materialize newly bound steps. Turn output into graph data with parse_output/report_finding. Map federation (OIDC/SAML role assumption) and cloud-to-on-prem pivots. Done when each cloud credential's reachable resources, roles, and federation edges are recorded as graph findings.`,
  opsec_sentinel:
