...
export function GraphNodeLinks({
  ...
  return (
    <span className={cn('inline-flex items-center gap-1 text-[10px] font-mono', className)}>
      ...
      <button onClick={() => navigateToGraph(nodeId, 2)} className="rounded px-1 py-0.5 text-accent hover:bg-accent/10">Graph</button>
      <button onClick={() => navigateToEvidence(nodeId)} className="rounded px-1 py-0.5 text-accent hover:bg-accent/10">Evidence</button>
    </span>
  );
}
