...
export function CredentialsLens({ active = true }: { active?: boolean }) {
  ...
  const selected = searchParams.get('item');
  const byCredential = useMemo(() => groupPlaybooks(playbooks), [playbooks]);
  const now = Date.now();
...
export function PathsLens({ active = true }: { active?: boolean }) {
  ...
  return (
    <div className={cn('h-full min-h-0 flex-col', active ? 'flex' : 'hidden')} data-testid="paths-lens">
      <div className="flex min-h-12 flex-shrink-0 flex-wrap items-center gap-2 border-b border-border-subtle px-4 py-2">
        <SearchInput value={query} onChange={setQuery} placeholder="Source, objective, edge, or assumption" />
        <select value={optimize} onChange={event => setOptimize(event.target.value as typeof optimize)} className="h-8 rounded border border-border-subtle bg-surface px-2 text-[10px] outline-none"><option value="confidence">Optimize confidence</option><option value="stealth">Optimize stealth</option></select>
        ...
        <select value={lane} onChange={event => setLane(event.target.value as AttackPathLaneFilter)} className="h-8 rounded border border-border-subtle bg-surface px-2 text-[10px] outline-none"><option value="all">All lanes ({counts.all})</option>{ATTACK_PATH_GROUPS.map(group => <option key={group.key} value={group.key}>{group.label} ({counts[group.key]})</option>)}</select>
...
export function CredentialInspector({ credential, onClose }: { credential: ExportedNode; onClose: () => void }) {
  ...
  const [error, setError] = useState<string | null>(null);
  const [prepared, setPrepared] = useState<{ run: string; step: string; attempt: string; execution: Record<string, unknown> } | null>(null);
  const availableTabs = ['summary', 'relationships', 'activity', 'actions'];
  const requestedTab = searchParams.get('tab');
  const tab = requestedTab && availableTabs.includes(requestedTab) ? requestedTab : 'summary';
  ...
  const act = async (key: string, operation: () => Promise<PlaybookRun | { run: PlaybookRun; execution: Record<string, unknown> }>) => {
    ...
    try {
      ...
      updateRun(run);
      if (claim.execution) setPrepared({ run: run.run_id, step: String(claim.execution.playbook_step_id || ''), attempt: String(claim.execution.playbook_attempt_id || ''), execution: claim.execution });
      else setPrepared(null);
    } catch (cause) { setError(cause instanceof Error ? cause.message : 'Playbook action failed.'); }
    finally { setBusy(null); }
  };

  return <WorkspaceInspector label="Credential inspector" title={String(credential.label || credential.id)} identifier={credential.id} tabs={availableTabs.map(value => ({ value, label: value[0].toUpperCase() + value.slice(1) }))} activeTab={tab} onTabChange={nextTab => { const next = new URLSearchParams(searchParams); if (nextTab === 'summary') next.delete('tab'); else next.set('tab', nextTab); setSearchParams(next); }} onClose={onClose}>
...
function PlaybookActions({ runs, busy, error, prepared, onAction }: { runs: PlaybookRun[]; busy: string | null; error: string | null; prepared: { run: string; step: string; attempt: string; execution: Record<string, unknown> } | null; onAction: (key: string, operation: () => Promise<PlaybookRun | { run: PlaybookRun; execution: Record<string, unknown> }>) => Promise<void> }) {
