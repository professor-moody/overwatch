...
- **Use one shared daemon by default** — `npm run setup` creates the HTTP MCP wiring and Claude hooks without replacing an existing engagement; then run `npm run daemon:start` and `npm run doctor`. The start command builds stale output only after proving no runtime owns the state. Use `npm run setup:stdio` only for an intentional Claude-only compatibility session with no dashboard/CLI workers. See `docs/getting-started.md`.
